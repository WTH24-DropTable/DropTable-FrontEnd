...
if (user!==null){
  ...
  return (
    ...
    { classAttendance.map((lesson,index) => (
      ...
      >
      <div>
      <span className="text-white">{lesson.class}</span>
      </div>
      <span
      className={`${
        ( lesson.data.attended.length>= 0.7*(lesson.data.attended.length+lesson.data.absent.length)) ? "text-green-500" : "text-red-500"
      }`}
      >
      {lesson.data.attended.length}/{lesson.data.attended.length+lesson.data.absent.length}
      </span>
      </div>
    ))}
    </div>
